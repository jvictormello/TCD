 ...
 */
public class UmaSecaoCortePerpendicularController {
...
public double ValorMyd(int fyd, double d){   
   double myd = (0.3*(fyd)*((Math.pow(d, 2.6)))); //Multiplica-se o d por 10 para deixar o momento em N.mm
    return myd;
}
...
public double EmbMenorPeca(double falfa1,double t1,double d){
    double rd1 = (falfa1)*t1*d; //Equação multiplicada por 10 para resultar em (N)
    return rd1;
}

//Cálculo EQ 25 - Embutimento maior peça de madeira
public double EmbMaiorPeca(double falfa1,double t2,double d, double beta){
    double rd2 = (falfa1)*t2*d*beta; //Equação multiplicada por 10 para resultar em (N)
    return rd2;

}

//Cálculo EQ 26 - Embutimento nas duas peças
public double EmbDuasPecas(double falfa1, double d, double t1, double t2, double beta, double valorFaxrk, double valorFaxrkPrego){ //Multiplica-se por 10 para igualar as unidades e resultar em (N)
    double johansen3 = (((falfa1*t1*d)/(1+beta))*((Math.sqrt(beta+(2*(Math.pow(beta, 2))*(1+(t2/t1)+(Math.pow(t2/t1, 2))))+(Math.pow(beta, 3)*Math.pow(t2/t1, 2))))-(beta*(1+(t2/t1)))));
...
public double DeformPinoMenorPeca(double falfa1,double t2,double d,double beta,double myd,double valorFaxrk, double valorFaxrkPrego){
    double johansen5 = (((falfa1*t2*d)/(1+(2*beta)))*((Math.sqrt((2*(Math.pow(beta, 2))*(1+beta))+((4*beta*(1+2*beta)*myd)/(falfa1*d*(Math.pow(t2, 2)))))-beta)));
